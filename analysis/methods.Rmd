...
This was split up into segments of a uniform length where each consecutive segment overlapped the previous segment by 70% of its length (`r fig_nums("segment_example", display = "cite")`).
...
fig_nums("segment_example", "Three overlapping segments from the full bee's path.", display = F)
```
`r fig_nums("three_segments")`
